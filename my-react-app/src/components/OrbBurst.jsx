...
function frame(t, P, out) {
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    out.push(
      spin([q[0] * r, q[1] * r, q[2] * r, 0.5 + 1.3 * (1 - u), f, u < 0.12 ? P.acc : P.dot], TAU * t * 0.25, 0.35)
    );
  }
}
...
function __OriginkitBase_OrbBurst(props) {
  const {
    style,
    dotColor = '#F4F1EA',
    accentColor = '#00FFE5',
  ...
  useEffect(() => {
    ...
    if (!ctx) {
      console.error('OrbBurst: 2D context unavailable');
      return undefined;
    }
...
export default function OrbBurst(props = {}) {
  return <__OriginkitBase_OrbBurst {...__originkitPresetProps} {...props} />;
}
